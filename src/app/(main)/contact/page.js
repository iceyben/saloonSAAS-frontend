...
const WHATSAPP = process.env.NEXT_PUBLIC_WHATSAPP || "250700000000";
...
export default function ContactPage() {
  ...
  return (
    <div style={{ paddingTop: "64px" }}>
      <section style={{ padding: "80px 24px 60px", background: "linear-gradient(135deg, var(--cream), var(--rose-blush))", textAlign: "center", borderBottom: "1px solid var(--border)" }}>
      ...
      <section style={{ padding: "80px 24px", background: "var(--cream)" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: "80px" }} className="contact-grid">
          <div>
            <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "28px", fontWeight: 300, marginBottom: "32px" }}>How to Reach Us</h2>
            {[
              { icon: <Phone size={20} />,          label: "Phone",     value: `+${WHATSAPP}`,       href: `tel:+${WHATSAPP}` },
              { icon: <MessageCircle size={20} />,  label: "WhatsApp",  value: "Chat with us now",   href: `https://wa.me/${WHATSAPP}` },
              { icon: <Clock size={20} />,          label: "Hours",     value: "Mon–Fri 8:00–18:00\nSat 9:00–16:00\nSun Closed", href: null },
            ].map((item, i) => (
            ...
            <div style={{ marginTop: "32px", borderRadius: "4px", overflow: "hidden", border: "1px solid var(--border)" }}>
              <iframe
