...
function BookingContent() {
  ...
  return (
    <div style={{ paddingTop: "72px", minHeight: "100vh", background: "var(--cream)" }}>
      {/* Header */}
      <section style={{ padding: "60px 40px 40px", background: `linear-gradient(135deg, var(--cream), var(--rose-blush))`, borderBottom: "1px solid var(--border)", textAlign: "center" }}>
        <div className="section-tag" style={{ justifyContent: "center" }}>Appointment</div>
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(36px, 4vw, 56px)", fontWeight: 300 }}>Book Your Visit</h1>
            ...
            {["Service", "Date & Time", "Your Details"].map((label, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <div style={{
                  ...
                  background: step > i + 1 ? "var(--rose)" : step === i + 1 ? "var(--rose)" : "var(--border)",
                  color: step >= i + 1 ? "white" : "var(--dark-mid)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: "12px", fontWeight: 500, transition: "all 0.3s",
                }}>
                  {step > i + 1 ? "✓" : i + 1}
                </div>
                <span style={{ fontSize: "12px", fontWeight: 400, letterSpacing: "0.06em", textTransform: "uppercase", color: step === i + 1 ? "var(--rose)" : "var(--dark-mid)" }}>
                  {label}
                </span>
        ...
        <AnimatePresence mode="wait">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                {services.map(s => (
                  <div key={s.id} onClick={() => { setSelectedService(s); setStep(2); }}
                    style={{
                      background: "white", border: `2px solid ${selectedService?.id === s.id ? "var(--rose)" : "var(--border)"}`,
                      borderRadius: "4px", padding: "24px", cursor: "pointer", transition: "all 0.2s",
                    }}
                  ...
                  >
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <span style={{ display: "flex", alignItems: "center", gap: "4px", fontSize: "12px", color: "var(--dark-mid)" }}>
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "32px" }}>
                <button onClick={() => setStep(1)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--dark-mid)" }}>
                  <ChevronLeft size={20} />
                </button>
                <div>
                  ...
                  {dates.map((d, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => { setSelectedDate(iso); setSelectedSlot(""); }}
                        style={{
                          padding: "12px 16px", borderRadius: "2px", cursor: "pointer",
                          border: `1px solid ${isSelected ? "var(--rose)" : "var(--border)"}`,
                          background: isSelected ? "var(--rose)" : "white",
                          color: isSelected ? "white" : "var(--dark)", transition: "all 0.15s",
              ...
              {selectedDate && (
                <div style={{ marginBottom: "40px" }}>
                  ...
                  {slotsLoading ? (
                    <div style={{ fontSize: "14px", color: "var(--dark-mid)" }}>Loading available slots...</div>
                  ) : slots.length === 0 ? (
                    <div style={{ fontSize: "14px", color: "var(--dark-mid)", padding: "20px", background: "white", borderRadius: "4px", border: "1px solid var(--border)" }}>
                      ...
                      {slots.map((slot, i) => (
                        <button key={i} onClick={() => setSelectedSlot(slot)}
                          style={{
                            ...
                            border: `1px solid ${selectedSlot === slot ? "var(--rose)" : "var(--border)"}`,
                            background: selectedSlot === slot ? "var(--rose)" : "white",
                            color: selectedSlot === slot ? "white" : "var(--dark)",
                            fontFamily: "var(--font-sans)", fontSize: "13px", transition: "all 0.15s",
                          }}>
                          {formatTime(slot)}
                        </button>
                      ))}
              ...
              <button onClick={() => setStep(3)} disabled={!selectedSlot}
                ...
                style={{ opacity: selectedSlot ? 1 : 0.4, cursor: selectedSlot ? "pointer" : "not-allowed" }}>
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "32px" }}>
                <button onClick={() => setStep(2)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--dark-mid)" }}>
                  <ChevronLeft size={20} />
                </button>
                <div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                  <div>
                    <label style={{ display: "block", fontSize: "12px", fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--dark-mid)", marginBottom: "8px" }}>
                      Full Name *
                    </label>
                ...
                <div>
                  ...
                  <textarea {...register("notes")} className="input-salon" rows={3} placeholder="Any special requests or notes..." style={{ resize: "vertical" }} />
                </div>

                {/* Payment method */}
                <div>
                  <label style={{ display: "block", fontSize: "12px", fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--dark-mid)", marginBottom: "12px" }}>
                    Payment Method *
                  </label>
                  <div style={{ display: "flex", gap: "12px" }}>
                    {[
                      { value: "CASH", label: "Pay in Cash", desc: "Pay when we arrive" },
                      { value: "MOMO", label: "Mobile Money", desc: "Upload payment screenshot" },
                    ].map(opt => (
                      <label key={opt.value} style={{
                        flex: 1, padding: "16px", border: `2px solid ${paymentMethod === opt.value ? "var(--rose)" : "var(--border)"}`,
                        borderRadius: "4px", cursor: "pointer", background: paymentMethod === opt.value ? "var(--rose-blush)" : "white",
                        transition: "all 0.2s",
                      }}>
                ...
                {paymentMethod === "MOMO" && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>
                    ...
                    <label style={{
                      ...
                      padding: "32px", border: "2px dashed var(--border)", borderRadius: "4px",
                ...
                <div style={{ background: "white", border: "1px solid var(--border)", borderRadius: "4px", padding: "24px" }}>
                  ...
                  {[
                    ...
                    <div key={row.label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--border)", fontSize: "14px" }}>
                      ...
                      <span style={{ fontWeight: 400, color: "var(--dark)" }}>{row.value}</span>
                    </div>
                  ))}
                </div>
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ textAlign: "center", padding: "60px 0" }}>
              <div style={{ color: "var(--rose)", marginBottom: "24px" }}>
                <CheckCircle size={64} strokeWidth={1} />
              </div>
              <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "40px", fontWeight: 300, marginBottom: "16px" }}>
                Booking Received!
              </h2>
              <p style={{ fontSize: "15px", color: "var(--dark-mid)", fontWeight: 300, maxWidth: "460px", margin: "0 auto 12px", lineHeight: "1.8" }}>
                Thank you, <strong>{booking?.customerName}</strong>! Your appointment request has been received and is awaiting confirmation.
              </p>
              <p style={{ fontSize: "14px", color: "var(--rose)", marginBottom: "40px" }}>
                You'll receive an SMS confirmation at {booking?.customerPhone} once approved.
              </p>
              <div style={{ display: "flex", gap: "16px", justifyContent: "center" }}>
                <a href="/" className="btn-outline">Back to Home</a>
                <a href="/booking" className="btn-primary" onClick={() => { setStep(1); setSelectedService(null); setSelectedDate(""); setSelectedSlot(""); }}>
                  Book Another
                </a>
              </div>
            </motion.div>
